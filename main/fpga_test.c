...
static bool
soc_message(ICE40* ice40, uint8_t cmd, uint32_t param, uint32_t *resp, TickType_t ticks_to_wait)
{
    esp_err_t res;
    uint8_t data_tx[6];
    uint8_t data_rx[6];
...
static bool
test_bitstream_load(ICE40* ice40, uint32_t *rc)
{
    esp_err_t res;
...
static bool
test_spi_loopback_one(ICE40* ice40)
{
    esp_err_t res;
    uint8_t data_tx[257];
    uint8_t data_rx[258];
...
static bool
test_spi_loopback(ICE40* ice40, uint32_t *rc)
{
    int i;
...
static bool
test_soc_loopback(ICE40 *ice40, uint32_t *rc)
{
    /* Execute command */
...
static bool
test_uart_loopback(ICE40* ice40, uint32_t *rc)
{
...
static bool
test_psram(ICE40* ice40, uint32_t *rc)
{
    /* Execute command */
...
static bool
test_irq_n(ICE40* ice40, uint32_t *rc)
{
    esp_err_t res;
...
static bool
test_lcd_mode(ICE40* ice40, uint32_t *rc)
{
    esp_err_t res;
    bool ok;
...
static bool
test_pmod_open(ICE40* ice40, uint32_t *rc)
{
    /* Execute command */
...
static bool
test_pmod_plug(ICE40* ice40, uint32_t *rc)
{
    /* Execute command */
...
static bool
test_lcd_init(ICE40* ice40, uint32_t *rc)
{
    /* Execute command */
...
static bool
wait_button(xQueueHandle buttonQueue)
{
...
static bool
run_test(ICE40* ice40, pax_buf_t* pax_buffer, const pax_font_t *font, ILI9341* ili9341, int line,
