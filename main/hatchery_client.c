...
#define NEXT                           \
    process_categories->cl = __LINE__; \
    return;                            \
    case __LINE__:;

    switch (process_categories->cl) {
        case 0:
...
#define NEXT                     \
    process_apps->cl = __LINE__; \
    return;                      \
    case __LINE__:;

    switch (process_apps->cl) {
        case 0:
...
#define NEXT                    \
    process_app->cl = __LINE__; \
    return;                     \
    case __LINE__:;

    switch (process_app->cl) {
        case 0:
            ...
            if (state == json_cb_open_object) {
                NEXT while (state == json_cb_string) {
                    if (string_appender_compare(&parser->string_appender, "slug") == 0) {
                        ...
                        NEXT if (state == json_cb_open_array) {
                            NEXT while (state == json_cb_open_object) {
